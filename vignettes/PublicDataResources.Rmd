...
Gene Expression Omnibus [GEO](https://www.ncbi.nlm.nih.gov/geo), the NCI Genomic Data
...
interfaces to the NCBI's Gene Expression Omnibus, Genomic Data Commons, and PharmacoDB. It additionally introduces curated resources 
...
nodes and edges may have Properties that supply instance details. ](`r system.file(package='PublicDataResources', 'vignettes', 'all_nodes_040318.png')`)
...
![A screenshot of an example query of the GDC repository portal.](`r system.file(package='PublicDataResources', 'vignettes', 'gdc_filtering.png')`)
...
Load packages:
```{r, cache=FALSE, echo=FALSE, results='hide', eval=FALSE}
...
```{r loadLibrary10, message=FALSE, eval=FALSE}
library(PharmacoGx)
```
...
[Structure of the PharmacoSet class](`r system.file(package='PublicDataResources', 'vignettes', 'PharmacoSet_structure.png')`)
...
```{r availablePSets, message=FALSE, eval=FALSE}
...
[Schematic view of the drug sensitivity datasets.](`r system.file(package='PublicDataResources', 'vignettes', 'Drug_sensitivity.png')`)

```{r availableSensitivityPSets, message=FALSE, eval=FALSE}
...
[Schematic view of drug perturbation datasets](`r system.file(package='PublicDataResources', 'vignettes', 'Drug_perturbation.png')`)

```{r availablePerturbationPSets, message=FALSE, eval=FALSE}
...
The [Biomarker discovery from large pharmacogenomics datasets](https://bioconductor.github.io/BiocWorkshops/biomarker-discovery-from-large-pharmacogenomics-datasets.html) workshop demonstrates analyses of `r BiocStyle::Biocpkg("PharmacoGx")` data.
